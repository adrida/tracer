...
class Router:
    ...
    def _route(self, X: np.ndarray):
        """Vectorized routing shared by the batch and async paths.

        Returns ``(preds, handled, stage_id, scores)`` like :func:`route_scored`,
        then applies the same out-of-distribution safety net as :meth:`predict`:
        rows far from the training distribution are force-deferred even when the
        surrogate is confident. Keeps the batch/async/batcher results consistent
        with single-sample :meth:`predict`.
        """
        preds, handled, stage_id, scores = route_scored(self._stages, X)
        if self._ood_gate is not None and np.asarray(handled).any():
            handled = np.asarray(handled) & ~self._ood_flags(X, preds)
        return preds, handled, stage_id, scores
    ...
    def predict_batch(
        ...
        self._check_dim(X)
        preds, handled, stage_id, scores = self._route(X)
